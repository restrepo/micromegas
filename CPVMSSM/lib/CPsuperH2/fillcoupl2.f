...
C! Micromegas Print      
      do KK=1,4
        write(321,fmt='(A2,I1,2x,1PE16.8)') 'la',KK,LR_H(KK)
      enddo
      do KK=1,3
      write(321,fmt='(A2,I1,A1,2x,1PE16.8)')
     . 'la',KK+4,'r',DREAL(LC_H(KK))
      write(321,fmt='(A2,I1,A1,2x,1PE18.8)')
     .'la',KK+4,'i',DIMAG(LC_H(KK))
      enddo
...
*-------------------------------------------------------------------------*
     ...
     . )
      ...
      ENDIF
*----
*      print*,'CKTAU_H,  TB*|CKTAU_H|',CKTAU_H,TB_H*CDABS(CKTAU_H)
...
*      print*,'CKB_H and CKBEW_H',CKB_H,CKBEW_H
     ...
     .            ,0.D0)
C! Micromegas Print
C Td3= (\Delat{hd}/hd)*sb/( cb*(1+\delta{hd}/hd) +  sb*\Dealta{hd}/hd )
      Td3=CKB_SUM*TB_H/(1.D0+CKB_SUM*TB_H)
      write(321 ,FMT='(A4,2x,1PE16.8)') 'Td3r',DREAL(Td3)
      write(321 ,FMT='(A4,2x,1PE16.8)') 'Td3i',DIMAG(Td3)
C! End of Micromegas Print
...
*|--->Higgs-top-top                                       [NC=25,26,27]
     ...
     .            ,0.D0)
C! Micromegas Print
C Tu3= (\Delat{hu}/hu)*cb/( sb*(1+\delta{hu}/hu) +  cb*\Dealta{hu}/hu )
      Tu3= CKT_H/TB_H/(1.D0+CKT_H/TB_H)
      write(321 ,FMT='(A4,2x,1PE16.8)') 'Tu3r',DREAL(Tu3)
      write(321 ,FMT='(A4,2x,1PE16.8)') 'Tu3i',DIMAG(Tu3)
C! ================                                                                                              
...
*|--->Higgs-photon-photon                                  [NC=88,89]
      CALL HPP(IH,M_C,MCH,HMASS,STMASS,SBMASS,STAUMASS,NCMAX,NHC_H
...
*
      ...
      SUBROUTINE HPP(IH,M_C,MCH,HMASS,STMASS,SBMASS,STAUMASS
...
*        print*,'b-quark pole mass',I,AS_MB,MB_POLE,MB_POLE_N
      ...
      ENDDO
  99  CONTINUE
*      print*,'As(mb^pole), mb(mb^pole), mb^pole :',as_mb,mb_mb,mb_pole
*c-quark pole mass
      MC_POLE=MCMT_H
      DO I=1,NQMAX
...
*        print*,'c-quark pole mass',I,AS_MC,MC_POLE,MC_POLE_N
      ...
      ENDDO
  98  CONTINUE
*      print*,'As(mc^pole), mc(mc^pole), mc^pole :',as_mc,mc_mc,mc_pole
*
...
*      print*,'SQRTS,AS(SQRTS) =',SQRTS,AS_S
*      print*,' > MQ(SQRTS)    :',MT_S,MB_S,MC_S,MS_S
*      if(ih.eq.1) write(*,29) sqrts,as_s,mt_s,mb_s,mc_s,ms_s
*-----------------------------------------------------------------------
*=======================================================================
*
      IBOT = 16
...
*
      ...
      GPT = 0.D0
*
      SGGSM(1)= GB*GSB*V_H/MBMT_H*HC_FSF(HMASS**2/4.D0/MBMT_H**2)
      SGGSM(2)= GT*GST*V_H/MTMT_H*HC_FSF(HMASS**2/4.D0/MTMT_H**2)
      SGGSM(3)=SGGSM(1)+SGGSM(2)

      PGGSM(1)= GB*GPB*V_H/MBMT_H*HC_FPF(HMASS**2/4.D0/MBMT_H**2)
      PGGSM(2)= GT*GPT*V_H/MTMT_H*HC_FPF(HMASS**2/4.D0/MTMT_H**2)
