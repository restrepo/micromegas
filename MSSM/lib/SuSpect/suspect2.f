...
c  VERSION 2.41  
c  Last modifications : June 02, 2008
...
c      
      if(input.le.1.or.input.eq.11) then
      if(input.eq.0.and.ichoice(1).ne.10) goto 8765
...
c
      if(input.eq.2.or.ichoice(1).eq.1) then
      mhu2     = dmhu2
      mhd2     = dmhd2
c
      m1       = dm1
...
c
c  rename input parameters for SLHA reading special purpose
      if(input.eq.2.or.input.eq.0) then
      if(input.eq.0.and.ichoice(1).eq.10) goto 9876 
      mhu20 = mhu2
      mhd20 = mhd2
c
      msl0=msl
...
Cpukhov      write(*,'(a)')' RUN TERMINATED : OUTPUT in suspect2.out'
      CLOSE(NOUT)      
                 endif
...
c -------------------------------------------------------------------- c
      ...
      do i=1,200,1
         read(ninlha,'(a1)',end=9900,err=9900) line1
...
c               modselval = line3
 ...
 9900 continue
Cpukhov print*,'SU_read_leshouches: end of file'
      done = .true.
...
cc         else
      ...
      write(nout,51) 'EXTPAR','Input parameters'
         unlikely = -123456789d0
c! essai      if(ichoice(1).ne.2) extval(10) = unlikely
      if(ichoice(8).eq.1) extval(0) = scale
      extcom(0) ='EWSB scale'
      if(ichoice(6).eq.0) then
...
c
Cpukhov      write(*,'(a)')' OUTPUT in SLHA format in suspect2_lha.out '
