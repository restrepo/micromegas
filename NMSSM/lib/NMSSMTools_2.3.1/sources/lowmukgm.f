	SUBROUTINE LOWMUKGM(PAR,CHECK,IFAIL)

**********************************************************************	
* Subroutine to compute mu, k and mss (if MAFLAG=-2)
*         or xis (if MAFLAG=-3) at the scale QSTSB
*
...
* XIS is stored in COMMON/GMSUSYPAR
...
**********************************************************************
	...
	DOUBLE PRECISION B,MUOLD,KOLD,MXSOLD
	...
	DOUBLE PRECISION RL,RTOP,RBOT,RTAU,RG,ANOMQSTSB
...
*   Running MH1, MH2(, MSS) from Q2 to QSTSB:
	...
	IF(MAFLAG.EQ.-3)
     C	MSSX=MSS-COEF*(2.D0*LQ**2*(MH1X+MH2X+MSS+ALQ**2)
     C      +2.D0*KQ**2*(3.D0*MSS+AKQ**2))*DLOG(Q2/QSTSB)
...
* If DETM < 0: No solution to the minimization equations:

	IF(DETM.LE.0.D0) THEN
	 IFAIL=16
	 RETURN
	ENDIF
...
*  K at QSTSB
	...
	KQ=LQ*(B-ALQ-MUP-(LQ*XIF+M3H)/MUQ)/MUQ	
...
*  MSSX = MS at QSTSB, stored in COMMON/QMHIGGS:
	...
	MSSX=-LQ**2*(H1Q**2+H2Q**2) - 2.D0*NUQ**2
     C	     +LQ**2*H1Q*H2Q/MUQ*(ALQ+2.D0*NUQ+MUP) - NUQ*AKQ
     C	     -KQ*XIF*(2.D0+MUP/NUQ)-MUP**2-3.D0*MUP*NUQ
     C	     -MSP-LQ*XIS/MUQ
...
*  NEW XIS:
	...
	XIS=LQ*(H1Q*H2Q*(ALQ+2.D0*NUQ+MUP)-MUQ*(H1Q**2+H2Q**2))
     C	    -MUQ/LQ*(MSSX+2.D0*NUQ**2+NUQ*AKQ+KQ*XIF*(2.D0+MUP/NUQ)
     C	    +MUP**2+3.D0*MUP*NUQ+MSP
     C	    -ct*LQ**2*H1Q*H2Q/MUQ*Xt*gmt
     C	    -cb*LQ**2*H1Q*H2Q/MUQ*Xb*gmb)
	
	ENDIF

*  CHECK of MU, K and MS or XIS against the previous value

	IF(MAFLAG.EQ.-2)THEN
	 CHECK=MAX(((MUOLD-PAR(4))/MAX(DABS(PAR(4)),DABS(MUOLD)))**2,
     C     ((KOLD-PAR(2))/MAX(DABS(PAR(2)),DABS(KOLD)))**2,
     C	   ((MXSOLD-MSS)/MAX(DABS(MSS),DABS(MXSOLD)))**2)
	ELSE
	 CHECK=MAX(((MUOLD-PAR(4))/MAX(DABS(PAR(4)),DABS(MUOLD)))**2,
     C     ((KOLD-PAR(2))/MAX(DABS(PAR(2)),DABS(KOLD)))**2,
     C	   ((MXSOLD-XIS)/MAX(DABS(MSS),DABS(MXSOLD)))**2)
	ENDIF

*   Approximate value for the MSSM-like CP odd Higgs mass squared MA:

	PAR(23)=DSQRT(MAX((MUQ*B+M3H+MUQ*MUP+LQ*XIF)
     C    *(tanbQ+1.D0/tanbQ),0.D0))
