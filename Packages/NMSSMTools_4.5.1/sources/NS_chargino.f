...
c chi+_1/chi+_2 --> sneutrino_e + e+
      ...
      do i=1,2,1
         if(asne1.le.amchar(i)) then
            charsnel(i)=g2s/2d0*alsne(1,i)**2*
...
c chi+_1/chi+_2 --> selectron_L+ + neutrino_e
      ...
      do i=1,2,1
         if(ase1.le.amchar(i)) then
            charsell(i)=g2s/2d0*(ale(1,i)**2)*
     .           (amchar(i)**2-ase1**2)*NS_lamb(0d0,ase1/amchar(i))
     .           /(16*pi*amchar(i))
         else
            charsell(i)=0d0
         endif
...
c chi+_1/chi+_2 --> selectron_R+ + neutrino_e
      ...
      do i=1,2,1
         if(ase2.le.amchar(i)) then
            charselr(i)=g2s/2d0*(ale(2,i)**2)*
     .           (amchar(i)**2-ase2**2)*NS_lamb(0d0,ase2/amchar(i))
     .           /(16*pi*amchar(i))
         else
            charselr(i)=0d0
         endif
...
c chi+_1/chi+_2 --> sdownl* + up
     ...
     .           NS_lamb(0d0,asdown1/amchar(i))/(16*pi*amchar(i))
         else
            charsdownl(i)=0d0
         endif
      end do
...
c chi+_1/chi+_2 --> sdownr* + up
     ...
     .           NS_lamb(0d0,asdown2/amchar(i))/(16*pi*amchar(i))
         else
            charsdownr(i)=0d0
         endif
      end do
...
c chi+_1/chi+_2 --> supl + downbar
     ...
     .           (amchar(i)**2-asup1**2)*
     .           NS_lamb(0d0,asup1/amchar(i))/(16*pi*amchar(i))
         else
            charsupl(i)=0d0
         endif
      end do
...
c chi+_1/chi+_2 --> supr + downbar
     ...
     .           (amchar(i)**2-asup2**2)*
     .           NS_lamb(0d0,asup2/amchar(i))/(16*pi*amchar(i))
         else
            charsupr(i)=0d0
         endif
      end do
...
c -------------------------------------------------------------------- c
     ...
     .        2d0*charsupl(i)+2d0*charsupr(i)+2d0*charsdownl(i)+
     .        2d0*charsdownr(i)+2d0*charsnel(i)+charsn1(i)+
     .        charsn2(i)+2d0*charsell(i)+2d0*charselr(i)+
...
c
     ...
     .       bto(i,nj)*blto(i,ni)*qr(nj,ni)*(-2d0)*
     .       dsqrt(xmun)*(xmuneut1+xmutau-uh) +
     .       ato(i,nj)*altau(i,ni)*ql(nj,ni)*2d0*
     .       dsqrt(xmutau)*(xmuneut1+xmun-th) +
     .       bto(i,nj)*blto(i,ni)*ql(nj,ni)*2d0*
     .       xmneut(nj)/xmchar(ni)*dsqrt(xmun)*(1d0+xmutau-th) +
     .       ato(i,nj)*altau(i,ni)*qr(nj,ni)*(-2d0)*
     .       xmneut(nj)/xmchar(ni)*dsqrt(xmutau)*(1d0+xmun-uh) +
     .       bto(i,nj)*altau(i,ni)*ql(nj,ni)*2d0*
     .       xmneut(nj)/xmchar(ni)*(1d0+xmuneut1-uh-th) +
     .       ato(i,nj)*blto(i,ni)*qr(nj,ni)*(-4d0)*
     .       xmneut(nj)/xmchar(ni)*dsqrt(xmun*xmutau) +
     .       ato(i,nj)*blto(i,ni)*ql(nj,ni)*2d0*
     .       dsqrt(xmun*xmutau)*(xmun+xmutau-th-uh) +
     .       altau(i,ni)*bto(i,nj)*qr(nj,ni)*2d0*
...
c
     ...
     .              (-x1*y1-x2*y2+x3*y3)
     .              +2d0*(alsne(k,ni)*anu(k,nj)*ale(i,ni)*ae(i,nj)
...
c
     ...
     .             2d0*alsne(i,ni)*anu(i,nj)*ol(nj,ni)*vwff*x2*y2
     .             -2d0*xmneut(nj)/xmchar(ni)*alsne(i,ni)*anu(i,nj)*
     .             or(nj,ni)*vwff*y3)
        endif
           end do
        else
