...
c -------------------------------------------------------------------- c
     ...
     .           2d0*neutsdownl(i)+2d0*neutsdownr(i)+2d0*
     .           neutsnel(i)+neutsn1(i)+neutsn2(i)+2d0*neutsell(i)+
     .           2d0*neutselr(i)+neutstau1(i)+neutstau2(i)+
...
*      
     ...
     .        uu(k,2))*(zz(i,3)*uu(k,1)-1d0/dsqrt(2d0)*zip*uu(k,2))
         glflgrfr = dsin(2d0*beta)*(uu(k,1)*vv(k,1)*(zz(j,4)*zz(i,3)-
...
c -------------------------------------------------------------------- c
     ...
     .xmusn(i).gt.1d0)Then
           xneutwsf=xneutwsf
     .       +g2s**2*4d0/dsne(i)/dw
     .      *(anu(i,ni)*alsne(i,nj)*ol(ni,nj)*2d0*vwff*x1*y1
     .       -alsne(i,nj)*anu(i,ni)*or(ni,nj)*2d0*vwff*
     .        xmchar(nj)/xmneut(ni)*y3)
      endif
         enddo
      else
...
c -------------------------------------------------------------------- c
     ...
     .       2d0*ato(i,ni)*alto(i,nj)*or(ni,nj)*(xmuneut1*(-xmutau
     .       +th-1d0)+th*(xmutau)+th*(1d0-th))
     .       +2d0*ato(i,ni)*alto(i,nj)*ol(ni,nj)*
     .       xmchar(nj)/xmneut(ni)*(uh+th-xmuneut1-1d0)+
     .       2d0*bto(i,ni)*alto(i,nj)*ol(ni,nj)*xmchar(nj)/xmneut(ni)*
     .       dsqrt(xmutau)*sgn(ni)*(uh-1d0)+
     .       4d0*bto(i,ni)*alto(i,nj)*or(ni,nj)*dsqrt(xmutau)*sgn(ni)*
     .       (th-xmuneut1)+
     .       1d0/xmuw*ato(i,ni)*alto(i,nj)*or(ni,nj)*(xmutau*
     .       (xmuneut1*(-uh+2d0)-th))+
     .       1d0/xmuw*ato(i,ni)*alto(i,nj)*ol(ni,nj)*
     .       xmchar(nj)/xmneut(ni)*(xmuneut1*(-xmutau)
     .       +xmutau*(uh+th-1d0))+
     .       1d0/xmuw*bto(i,ni)*alto(i,nj)*ol(ni,nj)*
     .       xmchar(nj)/xmneut(ni)*dsqrt(xmutau)*sgn(ni)*(xmuneut1*
     .       (-xmutau+th-1d0)+xmutau*(th)
     .       +uh*(-th+1d0)+th*(2d0-th)-1d0)+
     .       1d0/xmuw*bto(i,ni)*alto(i,nj)*or(ni,nj)*
     .       dsqrt(xmutau)*sgn(ni)*(
