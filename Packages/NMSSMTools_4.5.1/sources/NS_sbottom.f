...
c -------------------------------------------------------------------- c
     ...
     .              xintegsbsntau(i,j)+2d0*xintegsbsel(i,j)+
     .              xintegsbtstsb(i,j)+xintegsbtbstb(i,j)+
     .              xintegsbtaustnu(i,j)+2d0*xintegsbelstnu(i,j)+
     .              2d0*xintegsbupstdow(i,j)
            enddo
            sbottotmulti(i) = sbottotmulti(i)+2d0*xintegsbsnel(i)
            if(i.eq.2) then
...
*
      ...
      do k=1,2,1
       ...
       if ((amchar(k)+amt).gt.gmsb(ni).and.
     .(amchar(l)+amt).gt.gmsb(ni))Then
            NS_sbtsnutau=NS_sbtsnutau+1d0/dchi(k)/dchi(l)*(
