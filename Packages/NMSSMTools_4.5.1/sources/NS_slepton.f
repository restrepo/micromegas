...
c -------------------------------------------------------------------- c
     ...
     .        xmu2,xmu3,nx1t,ny1t,sum)
         xintegslstaustarltau=g2s**2/32d0/(2d0*pi)**3*MLR*sum
      else 
         xintegslstaustarltau=0d0
      endif
...
c -------------------------------------------------------------------- c
     ...
     .        xmu2,xmu3,nx1t,ny1t,sum)
         xintegslstaultau=g2s**2/32d0/(2d0*pi)**3*MLR*sum
      else 
         xintegslstaultau=0d0
      endif
        end
...
c -------------------------------------------------------------------- c
      ...
      if(MSL2.gt.(MSL1+2d0*MTAU)) then
         call NS_integ2(NS_stau2stau1star,NS_ax,NS_bx,NS_ay,NS_by,xmu1,
     .        xmu2,xmu3,nx1t,ny1t,sum)
         xintegstau2stau1star=g2s**2/32d0/(2d0*pi)**3*MSL2*sum
      else 
...
c -------------------------------------------------------------------- c
     ...
     .        xmu2,xmu3,nx1t,ny1t,sum)
         xintegstau2stau1=g2s**2/32d0/(2d0*pi)**3*MSL2*sum
      else 
...
c -------------------------------------------------------------------- c
     ...
     .        xmu2,xmu3,nx1t,ny1t,sum)
         xintegstau2stau1nn=g2s**2/32d0/(2d0*pi)**3*MSL2*sum
      else 
...
c -------------------------------------------------------------------- c
     ...
     .        xmu2,xmu3,nx1t,ny1t,sum)
         xintegsntaustau1star=g2s**2/32d0/(2d0*pi)**3*MSNT*sum
      else 
...
c -------------------------------------------------------------------- c
     ...
     .        xmu2,xmu3,nx1t,ny1t,sum)
         xintegsntaustau1=g2s**2/32d0/(2d0*pi)**3*MSNT*sum
      else 
...
c -------------------------------------------------------------------- c
     ...
     .        xmu2,xmu3,nx1t,ny1t,sum)
         xintegsntaustau1nutau=g2s**2/32d0/(2d0*pi)**3*MSNT*sum
      else 
...
c -> singularity from on-shell intermediate state
     ...
     .         +(atau(1,k)*atau(1,l))*
     .           (MNEU(k)*MNEU(l))/MSNT**2*(x1+x2-1d0+xstau1-xtau)
     .         +atau(1,k)*btau(1,l)*dsqrt(xneut(l)*xtau)*x1
...
c -------------------------------------------------------------------- c
     ...
     .        xmu2,xmu3,nx1t,ny1t,sum)
         xintegsellstau1star=g2s**2/32d0/(2d0*pi)**3*MLL*sum
      else 
...
c -------------------------------------------------------------------- c
     ...
     .        xmu2,xmu3,nx1t,ny1t,sum)
         xintegsellstau1=g2s**2/32d0/(2d0*pi)**3*MLL*sum
      else 
         xintegsellstau1=0d0
      endif
...
c -------------------------------------------------------------------- c
     ...
     .        xmu2,xmu3,nx1t,ny1t,sum)
         xintegsellstau1nutau=g2s**2/32d0/(2d0*pi)**3*MLL*sum
      else 
...
c -> singularity from on-shell intermediate state
     ...
     .          ((1d0-x1)*(1d0-x2)-xstau1+xtau)
     .         +(atau(1,k)*atau(1,l))*
     .           MNEU(k)*MNEU(l)/MLL**2*(x1+x2-1d0+xstau1-xtau)
     .         +atau(1,k)*btau(1,l)*dsqrt(xneut(l)*xtau)*x1
     .         +atau(1,l)*btau(1,k)*dsqrt(xneut(k)*xtau)*x1)
          endif
        enddo
...
c -------------------------------------------------------------------- c
     ...
     .        xmu2,xmu3,nx1t,ny1t,sum)
         xintegsnestau1star=g2s**2/32d0/(2d0*pi)**3*MNL*sum
      else 
...
c -------------------------------------------------------------------- c
     ...
     .        xmu2,xmu3,nx1t,ny1t,sum)
         xintegsnestau1=g2s**2/32d0/(2d0*pi)**3*MNL*sum
      else 
         xintegsnestau1=0d0
      endif
...
c -------------------------------------------------------------------- c
     ...
     .        xmu2,xmu3,nx1t,ny1t,sum)
         xintegsnestau1nutau=g2s**2/32d0/(2d0*pi)**3*MNL*sum
      else 
...
c -> singularity from on-shell intermediate state
     ...
     .         *((1d0-x1)*(1d0-x2)-xstau1+xtau)
     .         +(atau(1,k)*atau(1,l))*
     .          (MNEU(k)*MNEU(l))/MNL**2*(x1+x2-1d0+xstau1-xtau)
     .         +atau(1,k)*btau(1,l)*dsqrt(xneut(l)*xtau)*x1
     .         +atau(1,l)*btau(1,k)*dsqrt(xneut(k)*xtau)*x1)   
           endif
        enddo
