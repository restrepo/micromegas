...
*
* [24] Heavy Flavor Averaging Group,
*     www.slac.stanford.edu/xorg/hfag/rare/2013/bs/OUTPUT/HTML/bs_table1.html
*
* [25] Heavy Flavor Averaging Group,
*     www.slac.stanford.edu/xorg/hfag/rare/2013/radll/OUTPUT/HTML/radll_table7.html
*
* [26] Particle Data Group 2014,
*     http://pdg.lbl.gov/2014/reviews/rpp2014-rev-ckm-matrix.pdf
*
* [27] S. Aoki et al., ?Review of lattice results concerning low energy particle physics,?
*     arXiv:1310.8555,http://itpwiki.unibe.ch/flag
*
      ...
      DOUBLE PRECISION MT0,MTH,etaH,fBssqBs,fBdsqBd,fBs,dfBs
...
*    3.15 10^-3 < V_ub < 5.11 10^-3: (updated [26])
*      (V_ub)^2
      Vub2=(4.13d-3)**2
*      uncertainty on V_ub
      dVub=0.98d-3
...
*      updated to [27] f_Bs*sqrt(B_Bs)=(0.266 +/- 0.036) GeV  (2sigma)
      fBssqBs=0.266d0        ! GeV       f_Bs sqrt(B_Bs)
      dfBssqBs=0.036d0       ! GeV       uncertainty  (2sigma)

*      From [17]:
*      f_Bs*sqrt(B_Bs)/(f_Bd*sqrt(B_Bd))=1.216 +/- 0.0674 (2sigma)
*       so f_Bd*sqrt(B_Bd)=0.231*(1 +/- 0.1639) GeV  (2sigma)
*      updated to [27] f_Bd*sqrt(B_Bd)=(0.216 +/- 0.030) GeV  (2sigma)
      fBdsqBd=0.216d0          ! GeV       f_Bd*sqrt(B_Bd)
      dfBdsqBd=0.030d0         ! GeV       uncertainty  (2sigma)

*      From [15]:
*      f_B=(0.216 +/- 0.044) GeV (2sigma)
*      Update: [27] provides several lattice results with Nf=2,2+1,2+1+1
...
*      Delta Md from [22], 2 sigma bounds:
*      0.504ps-1 < DMd=0.510ps-1 < 0.516ps-1
      DMdexpmin=0.504d0
      DMdexpMax=0.516d0

*      Delta Ms from [23], 2 sigma bounds:
*      17.717ps-1 < DMs=17.761ps-1 < 17.805ps-1
      DMsexpmin=17.717d0
      DMsexpMax=17.805d0

*      From [24]: 1.2 10^-9 < BR(Bs->mu+mu-) < 5.2 10^-9 (95% C.L.)
      BRBMUMUexpmax=5.2d-9
      BRBMUMUexpmin=1.2d-9
      
*      BR(B+ -> tau+ nu) from [25], 2 sigma bounds:
*      0.70 10^-4 < BR(B+ -> tau+ nu)=1.14 10^-4 < 1.58 10^-4
      BRBTAUNUexpMax=1.58d-4
      BRBTAUNUexpmin=0.70d-4

*      BR(B -> Xs gamma) from [21], 2 sigma bounds:
...
*       .) Branching     
*     fBs was originally obtained from fBssqBs/sqBBs using
*      sqBBs=dsqrt(1.3d0)       !([10])       sqrt(B_Bs)
*     [27] provides several estimates for fBs from lattice computations:
*      fBs=(0.228+/-0.016) GeV (Nf=2), fBs=(0.2277+/-0.009) GeV (Nf=2+1),
*      fBs=(0.224+/-0.010) GeV (Nf=2+1+1),
*      We shall use the compromise fBs=(0.226+/-0.012) GeV
	fBs=0.226d0
	dfBs=0.012d0

      aux=GF**2*ALEM0**2/(s2tw**2*64d0*pi**3)*(5.3696d0)**5
     .     *1.454d0/(6.58211915d-13)*(fBs)**2
...
*      Comparison with experimental data (source [24])
...
*     2) Branching Ratio between 1GeV^2 and 6GeV^2
     ...
     .    +3d0/2d0*(cQ12+cQ22))
      ...
      BRBSllmax=(BRBSll-1.59d-6)*1.6d0+(1.59d0+.22d0)*1d-6
      BRBSllmin=(BRBSll-1.59d-6)*.4d0+(1.59d0-.22d0)*1d-6
...
*******************************************************************
      ...
      implicit none
      double precision x,y
      if(dabs(y-x).lt.1d-5)then
       if(dabs(y-1d0).lt.1d-5)then
       ffp=1d0/6d0
       else
...
*******************************************************************
      ...
      if(dabs(z-1d0).le.1d-5)then
        if(dabs(x-y).le.1d-5)then
         if(dabs(1d0-x).le.1d-5)then
         ggp=1d0/3d0
         else
         ggp=(x**2-2d0*dlog(x)*x-1d0)/(3d0*x-1d0+x**3-3d0*x**2)
         endif
        else
         if(dabs(x-1d0).le.1d-5)then
         ggp=(2d0*y**2*dlog(x)+4d0*y-1d0-3d0*y**2)
     .   /(-2d0+6d0*y+2d0*y**3-6d0*y**2)
         else if(dabs(1d0-y).le.1d-5)then
     ...
     .       +2d0*z**2+x-z+x**3*z**2-x**2*z**3-2d0*x**3*z+x**3)
         else if(dabs(x-1d0).le.1d-5)then
...
* Additional function Intpropa for BR(B-->Xs l+l-):
     ...
     .    +(bd2**3+bd2**2*bd1+bd2*bd1**2+bd1**3)/2d0)*(mb1s/fac)**4
      ELSE
       fac=(mb1s/mHmin)**2
       fca=(mb1s/mHmax)**2
       if(abs(mHmin-mHmax).gt.1d-4)then
