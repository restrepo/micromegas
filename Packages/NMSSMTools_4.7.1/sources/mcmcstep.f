	...
	IF(I.NE.31)X=X+DABS(PROB(I))
       ENDDO
       IF(X.NE.0d0)THEN
        IF(MCFLAG.EQ.0 .OR. MCFLAG.EQ.1)X=(1d0+X)*1d10
        IF(MCFLAG.EQ.2)X=(1d0+X)*1d15
       ENDIF
       IF(PROB(31).NE.0d0)THEN
        IF(MCFLAG.EQ.0 .OR. MCFLAG.EQ.2)X=X+(1d0+DABS(PROB(31)))*1d10
        IF(MCFLAG.EQ.1)X=X+(1d0+DABS(PROB(31)))*1d15
       ENDIF
      ENDIF
      X=X+FTSUSY(NSUSY+1)

c      IF(MCFLAG.EQ.1)THEN
c       IF(FTSUSY(NSUSY+1).GT.1d3)THEN
c        X=X+(FTSUSY(NSUSY+1)/1d3)*1d5
c       ENDIF
c       X=X+1d0
c      ELSEIF(MCFLAG.EQ.2)THEN
c       X=X+FTSUSY(NSUSY+1)
c      ELSE
c       X=X+1d0
c      ENDIF
