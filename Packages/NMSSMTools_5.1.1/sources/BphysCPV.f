      SUBROUTINE BOTTOMONIUM_CPV(PAR,PROB)
...
* Test on Upsilon(1S) -> A gamma (from CLEO)
     ...
     .  DELTA/3d0*(36d0*MBQM**2+MA**2)/(4d0*MBQM**2-MA**2))
       ...
       RMAX=0d0
       RMAX=dsqrt(2d0)*PI*ALEM0*CLEOTAU(MA)/(GF*MBQM**2*ZZ*C*BRYMUMU)
       IF(RMAX.NE.0d0)PROB(38)=DDIM(CBP(I)**2*BRLL(I)/RMAX,1d0)

       RMAX=0d0
       RMAX=dsqrt(2d0)*PI*ALEM0*CLEOMU(MA)/(GF*MBQM**2*ZZ*C*BRYMUMU)
...
*
     ...
     . BRBShllMaxexp
      ...
      DOUBLE PRECISION MCHH(2),etaH,D0B,D2B,etaS
