      SUBROUTINE HDECAY_lightA(PAR)

      IMPLICIT NONE

      INTEGER K,L,INDA,MIXPROB,N0

      DOUBLE PRECISION PAR(*),MA,GACHACHA(2,2),EPS,PI,aux
     ...
     .                 ,OMIX(4,4)
      ...
      DOUBLE PRECISION AAee,APIee,AEee,AEPee
      DOUBLE PRECISION AAmumu,APImumu,AEmumu,AEPmumu
      DOUBLE PRECISION AAtata
     ...
     .                 ,KinARhogam
      ...
      DOUBLE COMPLEX XC,TC,BC,CC,LC,MC,EC,CH1C,CH2C,CJA,CGA,FGG,CM
...
C  * A -> tautau
      AAtata=MTAU*CD(4)*DSQRT(dsqrt(2d0)*GF)
...
C  * A -> ee
     ...
     .          +OMIX(INDA,4)*AAee
      GamAee=aux**2/(8d0*Pi)*MA*dsqrt(max(0d0,1d0-4d0*MEL**2/MA**2))
