     ...
     .                 ,KinARhogam
      ...
      DOUBLE PRECISION METAC,DMC,CMIX,KinAPiDD
...
C	Coupling to photons / gluons
     ...
     .             +CB(IND)*FF(BC)/3d0+CD(IND)*(FF(LC)+FF(MC)+FF(EC))
     .                                               +CV(IND)*FV(WC))
     ...
     .        +CBP(IND)/3d0*FGG(BC)+CDP(IND)*(FGG(LC)+FGG(MC)+FGG(EC)))
...
C  * H -> tautau
      AHtata=MTAU*CD(IND)*DSQRT(dsqrt(2d0)*GF)
      GamHtata=AHtata**2/(8d0*Pi)*MH
     .               *dsqrt(max(0d0,1d0-4d0*MTAU**2/MH**2))**3

      AHtata=MTAU*CDP(IND)*DSQRT(dsqrt(2d0)*GF)
...
c      IF(MH.gt.2d0.and.MH.lt.2d0*MTAU)THEN
       ...
       IF(DMC.ne.0d0)THEN
        CMIX=datan(2d0*DMC
     .        /(MH**2-METAC**2+dsqrt((MH**2-METAC**2)**2+4d0*DMC**2)))
       ENDIF
