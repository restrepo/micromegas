...
*      PROB(74) =/= 0: excluded by HSM->ZA (LHC)
*      PROB(75) =/= 0: excluded by H/A->toptop (LHC)
*      PROB(76) =/= 0: excluded by A->ZHSM (LHC)
*      PROB(77) =/= 0: excluded by H/A->ZA/H (LHC)
*      PROB(78) =/= 0: excluded by H/A->HSMH/A (LHC)
*      PROB(78) =/= 0: excluded by H/A->HSMH/A (LHC)
*
************************************************************************
...
*********************************************************************
     ...
     .      x,dNdx,EMIN,NBIN
      ...
      WRITE(17,900) 2,"5.6.0      # Version number"
     ...
     . WRITE(17,900) 3,"# excluded by ggF/bb->H/A->tautau (LHC)"
      IF(PROB(52).NE.0d0)
     . WRITE(17,900) 3,
     ."# excluded by H_SM->AA->4leptons/2lept.+2b (LHC)"
      IF(PROB(53).NE.0d0)
     ...
     . WRITE(17,900) 3,"# DMK / epsK more than 2 sigma away"
      IF(PROB(63).NE.0d0)
     . WRITE(17,900) 3,"# excluded by H_SM->AA->4gam (LHC)"
      IF(PROB(64).NE.0d0)
     . WRITE(17,900) 3,
     ...
     . WRITE(17,900) 3,"# B_bsm(H_SM) more than 2 sigma away"
      IF(PROB(74).NE.0d0)
     . WRITE(17,900) 3,"# excluded by H_SM->Z+A->had (LHC)"
      IF(PROB(75).NE.0d0)
     . WRITE(17,900) 3,"# excluded by H/A->toptop (LHC)"
      IF(PROB(76).NE.0d0)
     . WRITE(17,900) 3,"# excluded by A->Z+H_SM->bb (LHC)"
      IF(PROB(77).NE.0d0)
     . WRITE(17,900) 3,"# excluded by H/A->Z+A/H (LHC)"
      IF(PROB(78).NE.0d0)
     . WRITE(17,900) 3,"# excluded by H/A->HSM+H/A (LHC)"
...
*  From IF(OUTFLAG.NE.1)
     ...
     .    " # (STOP/SBOTTOM MASSES)"
      ...
      WRITE(17,899) "# 1606.02266 Table 17, 1809.10733 Table 8,
     .  ATLAS-CONF-2020-027 Table 8(b)"
      WRITE(17,899) "# 0.914 < CV < 1.000 " ! electroweak couplings
      WRITE(17,899) "# 0.890 < CU < 1.248 " ! top quark
      WRITE(17,899) "# 0.690 < CB < 0.958 " ! b quark
      WRITE(17,899) "# 0.828 < CJ < 1.019 " ! gluons
      WRITE(17,899) "# 0.892 < CG < 1.049 " ! photons
      WRITE(17,899) "# 0.789 < CL < 0.989 " ! taus
...
*  From IF(OUTFLAG.NE.1)
      ...
      WRITE(18,900) 2,"5.6.0      # Version number"
     ...
     . 'BR(t -> ~t_2 ~chi_50)'
      ...
      WRITE(19,900) 2,"5.6.0      # Version number"
      IF(PROB(30).GT.0d0)
