...
*
* [13] LHCb and CMS collaborations, arXiv:1411.4413 [hep-ex].
*
* [14] CKMfitter Group (J. Charles et al.), 
*      Eur. Phys. J. C41, 1-131 (2005) [hep-ph/0406184], 
*      updated results and plots available at: http://ckmfitter.in2p3.fr
*      used here : ckmfitter.in2p3.fr/www/results/plots_ichep12/num/ckmEval_results_ICHEP12.pdf
*
* [15] H. Na, C. J. Monahan, C. T. H. Davies, R. Horgan, G. P.~Lepage 
*      and J. Shigemitsu,
*      ``The B and B_s Meson Decay Constants from Lattice QCD,''
*      Phys.  Rev. D 86 (2012) 034506
*      arXiv:1202.4914 [hep-lat].
*      A. Bazavov et al. (Fermilab Lattice and MILC Collaboration),
*      ``B- and D-meson decay constants from three-flavor lattice QCD,''
*      Phys. Rev. D 85 (2012) 114506
*      arXiv:1112.3051 [hep-lat].
*      P. Dimopoulos et al. [ETM Collaboration],
*      ``Lattice QCD determination of m_b, f_B and f_Bs with twisted 
*      mass Wilson fermions,''
*      JHEP 1201, 046 (2012)
*      arXiv:1107.1441 [hep-lat].
*
* [16] J. Beringer et al. (Particle Data Group), 
*      Phys. Rev. D86 (2012) 010001
*
* [17]  The Heavy Flavor Averaging Group,
*       ``Averages of b-hadron, c-hadron, and tau-lepton Properties as 
*       of early 2012,''
*       arXiv:1207.1158 [hep-ex].
*
* [18]  C. McNeile, C. T. H. Davies, E. Follana, K. Hornbostel 
*       and G. P. Lepage,
*       ``High-Precision $f_{B_s}$ and HQET from Relativistic Lattice 
*       QCD,''
*       Phys. Rev. D 85, 031503 (2012)
*       arXiv:1110.4510 [hep-lat].
*
* [19]  P. Gambino and P. Giordano, 
*       ``Normalizing inclusive rare B decays'', 
*       Phys. Lett. B669 (2008) 69–73, 
*       arXiv:0805.0271.
*
* [20] Heavy Flavor Averaging Group,
*     www.slac.stanford.edu/xorg/hfag/osc/PDG_2014/#DMD
*
* [21] Heavy Flavor Averaging Group,
*     www.slac.stanford.edu/xorg/hfag/osc/PDG_2014/#DMS
*
* [22] Heavy Flavor Averaging Group,
*     www.slac.stanford.edu/xorg/hfag/rare/2013/radll/OUTPUT/HTML/radll_table7.html
*
* [23] Heavy Flavor Averaging Group,
*      www.slac.stanford.edu/xorg/hfag/rare/2013/radll/btosg.pdf
      ...
      DOUBLE PRECISION ff88,Mch2,dff22,ff22p,ff27p
      ...
      DOUBLE PRECISION KC0,KT0,KBSM0,KC,KT,KCIM,KTIM,Ktot,dKBSM0
      DOUBLE PRECISION dKC,dKCr,dKCi,dKCIM,dKCIMr,dKCIMi
      DOUBLE PRECISION KIM,KBSM,KBSMIM
      DOUBLE PRECISION sc0,VVtu,VVtuim,Vbsg,rmu,dVVtu,dVVtuim
      DOUBLE PRECISION EPSew,HQET,BREMS,CCSL,dCCSL,dBREMS,dHQET
     ...
     .       BRBMUMUexpMin,BRBMUMUexpMax,BRBTAUNUexpmin,
     .       BRBTAUNUexpMax,BRSGexpmin,BRSGexpMax
      DOUBLE PRECISION sigRLbs,sigRLbd,sigLRbs,sigLRbd,BB0,BB1
      DOUBLE PRECISION C2LR,C1SLL,C1SRR,Ca,Cs,Cp,C70S0,sgn,dCa,dMT0
      DOUBLE PRECISION Vub2,fB,mBu,mBs,mBd,tauB,dtauB,tauBs,dtauBs,rh
     ...
     .       BRBtaunumax,BRBtaunumin
      DOUBLE PRECISION BRBSll,BRBSll1,BRBSll1min,BRBSll1max,BRBSll2,
     .       BRBSll2min,BRBSll2max,CQ12,CQ22,Intpropa
      DOUBLE PRECISION SGNT,SGNB,LAMBDA,AL,ATAU
     ...
     .      DMsmax,DMsmin,BRBMUMU,BRBMUMUmax,BRBMUMUmin,BRBtaunu,
     .      BRBtaunumax,BRBtaunumin,BRBSll,BRBSll1,BRBSll1min,
     .      BRBSll1max,BRBSll2,BRBSll2min,BRBSll2max
     ...
     .      BRSUSY,WIDTH
      COMMON/NOBUG/TANB,AAT,AAB,Mch2,SST,SSB,LAMBDA,AL,ATAU
...
*       Matching Scale sc0 = m_top(m_top)(MSbar)
      ...
      sc0=MT0
*       Uncertainty on m_top(m_top)(MSbar), with :
*       delta(asmt)=5.8d-4 (relevant around MT=172.5GeV), delta(MT)=1GeV     
      dMT0=dsqrt((1d0)**2/(1d0+4d0/(3d0*pi)*asmt+11d0/pi**2*asmt**2)  
     .   +(MT*(4d0/(3d0*pi)+2d0*11d0/pi**2*asmt))**2*(5.8d-4)**2)   
...
*       x Parameters (Squares of Mass Ratios)
      ...
      do l=1,6
       do k=1,2
       xTneu(k,l)=(ST(k)/MNEU(l))**2       !(m_St(k)/m_neu(i))^2
       xBneu(k,l)=(SB(k)/MNEU(l))**2       !(m_Sb(k)/m_neu(i))^2
       enddo
      enddo
      
*       CKM coefficients

      VVc=0.92842356d0   ! |V_cs.V_cb|/|V_ts.V_tb| see [16]; [14]:0.99253
      VVu=0.48173314d0   ! |V_ud.V_ub|/|V_td.V_tb| see [16]; [14]:0.39206
      Vtb2=(0.999154626d0)**2   ! (V_tb)^2 see [16] : dVtb=0.0002746
      Vcs2=(0.973d0)**2  ! (V_cs)^2 see [16]; [14]:0.97344**2
      Vud2=(0.97425d0)**2! (V_ud)^2 see [16]; [14]:0.97473**2
      

*      (V_ub)^2
      Vub2=(4.15d-3)**2 ! see [16]; |Vub| not semileptonic see [14]
      dVub=0.49d-3

*       From [14]: dvtd:+0.00047-0.00023 and dvtb:+0.000043-0.000025 ->
*       V_tb*V_td=(8.50 +0.47-0.23) 10^-3
*      (V_tb*V_td)^2 from [16]:
      VtdVtb2=(8.3929d-3)**2
*      uncertainty on (V_tb*V_td)^2
      dVtdVtb2=2d0*dsqrt(VtdVtb2)*5.995d-4
      
*       From [14]: dvts:+0.00129-0.00048 and dvtb:+0.000043-0.000025 ->
*       V_ts*V_tb=(39.99 +1.29-0.48) 10^-3
*      (Vtb.V_ts)^2 from [16]:
      VtsVtb2=(0.042863535d0)**2
*      uncertainty on (Vtb.V_ts)^2
      dVtsVtb2=2d0*dsqrt(VtsVtb2)*2.597817d-3

*      (V_ts.V_tb/V_cb)^2 from [16] (from [14]:0.04077)
      Vbsg=VtsVtb2/(0.0409d0)**2

*      V_us.V_ub/V_ts.V_tb, from [16] (from [14]:-0.008057+i0.017303)
      VVtu=-0.009187d0
      VVtuim=0.021399d0
      dVVtu=0.004d0
*      dVVtu : + 0.0038 - 0.0042
      dVVtuim=0.00305d0
*      dVVtuim : + 0.0030 - 0.0031
...
*      Muon mass
      ...
      mmu=0.1056583715d0 !GeV
      
*      Hadronic parameters :

*      From [18]: f_Bs=(0.225 +/- 0.004) GeV
      fBs=0.225d0
      dfBs=0.004d0
*      From [14]: B_Bs=1.3220 +/- 0.0436
      fBssqBs=0.2587d0       ! GeV       f_Bs sqrt(B_Bs)
*      Uncertainty =dsqrt(B*delf² + f²delB²/4B)
      dfBssqBs=0.0063d0      ! GeV       uncertainty

*      From first ref. in [15]:
*      f_B=(0.189 +/- 0.004) GeV
      fB=0.189d0             ! GeV       for B+ --> tau+ nu_tau
      dfB=0.004d0            ! GeV       uncertainty
      
*      From [15]+[14]: 
*      f_Bd*sqrt(B_Bd)=0.2148 +/- 0.0094 GeV
      fBdsqBd=0.2148d0       ! GeV       f_Bd*sqrt(B_Bd)
      dfBdsqBd=0.0094d0      ! GeV       uncertainty
      
*      Experimental data

*      Delta Md from [20], 2 sigma bounds:
*      0.504ps-1 < DMd=0.510ps-1 < 0.516ps-1
      DMdexpmin=0.504d0
      DMdexpMax=0.516d0

*      Delta Ms from [21], 2 sigma bounds:
*      17.717ps-1 < DMs=17.761ps-1 < 17.805ps-1
      DMsexpmin=17.717d0
      DMsexpMax=17.805d0      
      
*      BR(Bs->mu+mu-) from [13], 2 sigma bounds:
*      1.6 10^-9 < BR(Bs->mu+mu-)=2.8 10^-9 < 4.2 10^-9
      BRBMUMUexpmax=4.2d-9
      BRBMUMUexpmin=1.6d-9
      
*      BR(B+ -> tau+ nu) from [22], 2 sigma bounds:
*      0.70 10^-4 < BR(B+ -> tau+ nu)=1.14 10^-4 < 1.58 10^-4
      BRBTAUNUexpMax=1.58d-4
      BRBTAUNUexpmin=0.70d-4

*      BR(B -> Xs gamma) from [23], 2 sigma bounds:
*      2.99 10^-4 < BR(B -> Xs gamma)=3.43 10^-4 < 3.87 10^-4
      BRSGexpmin=2.99d-4
      BRSGexpMax=3.87d-4
...
*       a) epst1 = (epsilontilde as in[10])*tanb, epst2 (* tanb)
     ...
     .      +BB1(0d0,MGL**2,MDL**2,scR2))              !gluinos
      ...
      aux=0d0
      do l=1,6
      aux=aux+2d0/H1Q*MNEU(l)                      !neutralinos
     .   *((gg1/3d0*N(l,1)-gg2*N(l,2))*N(l,4)
     .       *BB0(0d0,MNEU(l)**2,MDL**2,scR2)
     .    +DSQRT(2d0)/3d0*gg1*N(l,1)*N(l,4)
     .       *BB0(0d0,MNEU(l)**2,MDL**2,scR2))
     .       +((MS0/H1Q)**2*N(l,4)**2
     .   +1d0/2d0*(gg1/3d0*N(l,1)-gg2*N(l,2))**2)
     .       *BB1(0d0,MNEU(l)**2,MDL**2,scR2)
     .       +(2d0/9d0*gg1**2*N(l,1)**2
     .   +(MS0/H1Q)**2*N(l,4)**2)
     .       *BB1(0d0,MNEU(l)**2,MDL**2,scR2)
      enddo
      ...
      aux=0d0
      do i=1,2
      aux=aux-2d0*MCH(i)*gg2/H1Q                    !charginos
...
*       b) epst3 (* tanb)
     ...
     . -BB0(0d0,MGL**2,SB(2)**2,scR2)))                  !gluinos
      ...
      do l=1,6
      aux=aux                                   !neutralinos
     .   +2d0*((CSB*(gg1/3d0*N(l,1)-gg2*N(l,2))
     .     +HBQ*dsqrt(2d0)*SSB*N(l,4))
     .      *(gg1/3d0*SSB*N(l,1)+HBQ/dsqrt(2d0)*CSB*N(l,4))
     .      *BB0(0d0,MNEU(l)**2,SB(1)**2,scR2)
     .      -(SSB*(gg1/3d0*N(l,1)-gg2*N(l,2))
     .     -HBQ*dsqrt(2d0)*CSB*N(l,4))
     .      *(gg1/3d0*CSB*N(l,1)-HBQ/dsqrt(2d0)*SSB*N(l,4))
     .      *BB0(0d0,MNEU(l)**2,SB(2)**2,scR2))*MNEU(l)/MB0
     .   +((dsqrt(2d0)*gg1/3d0*SSB*N(l,1)+HBQ*CSB*N(l,4))**2
     .   +(CSB/dsqrt(2d0)*(gg1/3d0*N(l,1)-gg2*N(l,2))
     .              +HBQ*SSB*N(l,4))**2)
     .      *BB1(0d0,MNEU(l)**2,SB(1)**2,scR2)
     .   +((dsqrt(2d0)*gg1/3d0*CSB*N(l,1)-HBQ*SSB*N(l,4))**2
     .   +(SSB/dsqrt(2d0)*(gg1/3d0*N(l,1)-gg2*N(l,2))
     .              -HBQ*CSB*N(l,4))**2)
     .      *BB1(0d0,MNEU(l)**2,SB(2)**2,scR2)
      enddo
      epst3=epst3+1d0/(32d0*pi**2)*aux

      aux=0d0
      do i=1,2                                  !charginos
      aux=aux-2d0*MCH(i)/H1Q
...
*       d) epsY13 (*Yt^2 tanb), epsY23 (*Yt^2 tanb)
      epsY13=0d0
      do i=1,2
      epsY13=epsY13-2d0/H1Q*U(i,2)*MCH(i)
     .       *(CST*(gg2*CST*V(i,1)-HTQ*SST*V(i,2))
     .    *BB0(0d0,MCH(i)**2,ST(1)**2,scR2)
     .       +SST*(gg2*SST*V(i,1)+HTQ*CST*V(i,2))
     .    *BB0(0d0,MCH(i)**2,ST(2)**2,scR2))
     ...
     .         *BB1(0d0,MCH(i)**2,MUL**2,scR2))
      ...
      epsY23=0d0
      do i=1,2
      epsY23=epsY23-2d0/H1Q*U(i,2)*MCH(i)
     .       *(CST*(gg2*CST*V(i,1)-HTQ*SST*V(i,2))
     .    *BB0(0d0,MCH(i)**2,ST(1)**2,scR2)
     .       +SST*(gg2*SST*V(i,1)+HTQ*CST*V(i,2))
     .    *BB0(0d0,MCH(i)**2,ST(2)**2,scR2))
...
*       WITHOUT the S dependent mixing angles
      ...
      sigRLbs=MB0*epsY32/(H2Q*(1d0+epst0)*(1d0+epst3))
      
      sigRLbd=MB0*epsY31/(H2Q*(1d0+epst0)*(1d0+epst3))

      sigLRbs=MS0*epsY23/(H2Q*(1d0+epst0)*(1d0+epst3))
     .  *(1d0+epst3+(epst2-epst3)*epsY32/epsY23)/(1d0+epst2)

      sigLRbd=MD*epsY13/(H2Q*(1d0+epst0)*(1d0+epst3))
...
*      Double Penguin Contributions to DMd
      ...
      mBd=5.27958d0 ! [16], p.84

*   - Wilson coefficients
      aux=0d0
      do i=1,3
      aux=aux+(SCOMP(i,2)-SCOMP(i,1)*tanb)**2
     . *sgn(SMASS(i)**2-mBd**2)/
     . dsqrt((SMASS(i)**2-mBd**2)**2+(SMASS(i)*WIDTH(i))**2)
      enddo
      aux=aux+(PCOMP(3,2)-PCOMP(3,1)*tanb)**2
     . *sgn(PMASS**2-mBd**2)/
     . dsqrt((PMASS**2-mBd**2)**2+(PMASS*WIDTH(4))**2)

      C2LR=-(4d0*pi/(GF*MW))**2*sigRLbd*sigLRbd*aux ! [10], p. 49
...
*      Results for DMd
      ...
      aux=GF**2*MW**2/(6d0*pi**2)*etaB*mBd*(fBdsqBd)**2
     .       *xt*VtdVtb2/(6.58211928d-13)

      DMd=aux*abs(DSM+DH+Dchid+DDP)


*      Error Estimate: (CKM and lattice QCD uncertainties)
*       * lattice QCD (sources [23,21]):
*       0.2054 GeV < fBd sqrt(BBd)=0.2148 GeV < 0.2242 GeV
*      * CKM factor (source [14]):V_tb*V_td=(8.50 +0.47-0.23) 10^-3
*       from [16]: 6.682 10^-3 < VtbVtd=7.476 10^-3 < 8.27 10^-3
*       * 30% on BSM (1st order QCD) contributions

*     First: 2 sigma "SM" (relative) error bars from Mt, CKM and
*       lattice uncertainties: (2sigma, added quadratiCALLy)
      DMdMax=(1d0+2*dsqrt((dVtdVtb2/VtdVtb2)**2
     .  +(2d0*dfBdsqBd/fBdsqBd)**2
     .  +(aux*3/4*(1+4*xt-5*xt**2+2*xt*(2+xt)*dlog(xt))
     .  /(DMd**2*(xt-1)**4))**2*(2*(xt*dMT0/MT0))**2))*DMd
      DMdmin=(1d0-2*dsqrt((dVtdVtb2/VtdVtb2)**2
     .  +(2d0*dfBdsqBd/fBdsqBd)**2
     .  +(aux*3/4*(1+4*xt-5*xt**2+2*xt*(2+xt)*dlog(xt))
     .  /(DMd**2*(xt-1)**4))**2*(2*(xt*dMT0/MT0))**2))*DMd

*      Total error bars, allowing for 10% theory error:

      DMdmax=DMdMax+2*0.1d0*DMd
      DMdmin=DMdmin-2*0.1d0*DMd

*      Comparison with experimental data (source [20]):
*      (Recall: 2 sigma bounds: 0.504ps-1 < DMd < 0.515ps-1)

      prob(34)=0d0

      IF(DMdmin.GE.DMdexpMax)THEN
      PROB(34)=DMdmin-DMdexpMax
      ENDIF
      IF(DMdmax.LE.DMdexpmin)THEN
      PROB(34)=DMdmax-DMdexpmin
      ENDIF
...
*   - Wilson coefficients
      ...
      aux=aux+(SCOMP(i,2)-SCOMP(i,1)*tanb)**2
     . *sgn(SMASS(i)**2-mBs**2)/
     . dsqrt((SMASS(i)**2-mBs**2)**2+(SMASS(i)*WIDTH(i))**2)
      enddo
      aux=aux+(PCOMP(3,2)-PCOMP(3,1)*tanb)**2
     . *sgn(PMASS**2-mBs**2)/
     . dsqrt((PMASS**2-mBs**2)**2+(PMASS*WIDTH(4))**2)
...
*      Results for DMs      
      ...
      aux=GF**2*MW**2/(6d0*pi**2)*etaB*mBs*(fBssqBs)**2
     .       *VtsVtb2*xt/(6.58211928d-13)
      
      DMs=aux*abs(DSM+DH+Dchis+DDP)

*      Error Estimate: (CKM and lattice QCD uncertainties)
*      * lattice QCD (from source [14]+[18]):
*       0.2524 GeV < fBs sqrt(BBs)=0.2587 GeV < 0.2650 GeV
*      * CKM factor (source [14]):
*      from [16]: 34.391 10^-3 < VtsVtb=38.181 10^-3 < 41.971 10^-3
*      * 30% on BSM (1st order QCD) contributions

*      First: 2 sigma "SM" error bars from Mt, CKM and
*      lattice uncertainties: (2sigma, added quadratiCALLy)
      DMsMax=(1d0+2*dsqrt((dVtsVtb2/VtsVtb2)**2
     .  +(2d0*dfBssqBs/fBssqBs)**2
     .  +(aux*3/4*(1+4*xt-5*xt**2+2*xt*(2+xt)*dlog(xt))
     .  /(DMs**2*(xt-1)**4))**2*(2*(xt*dMT0/MT0))**2))*DMs
      DMsmin=(1d0-2*dsqrt((dVtsVtb2/VtsVtb2)**2
     .  +(2d0*dfBssqBs/fBssqBs)**2
     .  +(aux*3/4*(1+4*xt-5*xt**2+2*xt*(2+xt)*dlog(xt))
     .  /(DMs**2*(xt-1)**4))**2*(2*(xt*dMT0/MT0))**2))*DMs
...
*      Total error bars, allowing for 30% theory error:
      ...
      DMsmax=DMsMax+2*0.1d0*DMs
      DMsmin=DMsmin-2*0.1d0*DMs

*      Comparison with experimental data (source [21])
*       (Recall: 2 sigma bounds: 17.717ps-1 < DMs < 17.805ps-1)

      prob(33)=0d0

      IF(DMsmin.GE.DMsexpMax)THEN
      PROB(33)=DMsmax-DMsexpMax
      ENDIF
      IF(DMsmax.LE.DMsexpmin)THEN
      PROB(33)=DMsmax-DMsexpmin
      ENDIF
...
*      Contributions to BR(Bs -> mu+ mu-)    (see [10,11])
      ...
      tauBs=1.497d0/(6.58211928d-13) ! [16], p.4,5,105
      dtauBs=0.015d0/(6.58211928d-13) ! [16], p.4,5,105
...
*       a) Wilson coefficients - SM contribution
     ...
     .   +(3d0*xt+2d0)/(xt-1d0)**2*dlog(xt))
      ...
      dCa=dabs((-4d0+3d0*xt+xt**3-6d0*xt*dlog(xt))
     .   /(8d0*(xt-1d0)**3))*2*(xt*dMT0/MT0)
     

*       b) Wilson coefficients - DP contribution
      aux=0d0
      do i=1,3
      aux=aux-(SCOMP(i,2)-SCOMP(i,1)*tanb)*SCOMP(i,1)
     . *sgn(SMASS(i)**2-mBs**2)/(cosb
     . *dsqrt((SMASS(i)**2-mBs**2)**2+(SMASS(i)*WIDTH(i))**2))
      enddo
      cs=gg2/(2d0*MW)*(pi/(GF*MW))**2*mmu/MB0*sigRLbs*aux

      aux=0d0
      aux=aux-(PCOMP(3,2)-PCOMP(3,1)*tanb)*PCOMP(3,1)
     .  *sgn(PMASS**2-mBs**2)/(cosb
     . *dsqrt((PMASS**2-mBs**2)**2+(PMASS*WIDTH(4))**2))
      cp=-gg2/(2d0*MW)*(pi/(GF*MW))**2*mmu/MB0*sigRLbs*aux

*       .) Branching ratio      

      aux=GF**2*ALEM0**2/(s2tw**2*64d0*pi**3)*mBs**5
     .     *tauBs*fBs**2*Vtsvtb2*dsqrt(1d0-4d0*(mmu/mBs)**2)

      BRBMUMU=aux*
     .      ((1d0-4d0*(mmu/mBs)**2)/(1d0+MS/Mb)**2*Cs**2
     .       +(Cp/(1d0+ms/mb)+2d0*mmu/mBs**2*Ca)**2)

*      Error Estimate: (Mt, CKM and lattice QCD uncertainties)
*      * lattice QCD (from source [14]+[18]):
*       0.2524 GeV < fBs sqrt(BBs)=0.2587 GeV < 0.2650 GeV
*      * CKM factor (source [14]):
*      from [16]: 34.391 10^-3 < VtsVtb=38.181 10^-3 < 41.971 10^-3
...
*       First: 2 sigma "SM" error bars from CKM and
*       lattice uncertainties: (2sigma, added quadratically)

      BRBMUMUMAX=(1d0+2*dsqrt((dVtsVtb2/VtsVtb2)**2
     .      +(2d0*dfBs/fBs)**2+(dtauBs/tauBs)**2
     .      +((2*aux*Ca*(2d0*mmu/mBs**2)**2+2*aux*2d0*mmu/mBs**2
     .      *Cp/(1d0+ms/mb))/BRBMUMU)**2*dCa**2))*BRBMUMU
      BRBMUMUmin=(1d0-2*dsqrt((dVtsVtb2/VtsVtb2)**2
     .      +(2d0*dfBs/fBs)**2+(dtauBs/tauBs)**2
     .      +((2*aux*Ca*(2d0*mmu/mBs**2)**2+2*aux*2d0*mmu/mBs**2
     .      *Cp/(1d0+ms/mb))/BRBMUMU)**2*dCa**2))*BRBMUMU
...
*      Total error bars, allowing for 10% theory error:
      ...
      BRBMUMUmax=BRBMUMUMAX+2*0.1d0*BRBMUMU
      BRBMUMUmin=BRBMUMUmin-2*0.1d0*BRBMUMU

*      Comparison with experimental data (source [13])
*    (Recall : 1.6 10^-9 < 2.8 10^-9 < 4.2 10^-9)

      prob(35)=0d0

      IF(BRBMUMUmin.GE.BRBMUMUexpMax)THEN
      PROB(35)=BRBMUMUmin/BRBMUMUexpMax-1d0
      ENDIF
      IF(BRBMUMUmax.LE.BRBMUMUexpMin)THEN
...
*                           following [12]
      tauB=1.641d0/(6.58211928d-13) ! [16], p.4,5,70
      dtauB=0.008d0/(6.58211928d-13) ! [16], p.4,5,70
      mBu=5.27925d0 ! [16], p.70
...
*   2 sigma bounds (source [22]):
*   0.70 10^-4 < BR(B+ -> tau+ nu)=1.14 10^-4 < 1.58 10^-4

*  hadronic parameter ([15]): 0.185 GeV < fB=0.189 GeV <0.193 GeV
*  CKM factor ([16]): 3.66 10^-3 < Vub=4.15 10^-3 < 4.64 10^-3
*  1.633 ps < tauB=1.641 ps < 1.648 ps

*  2 sigma (absolute) error bars, added quadratically, from CKM and
*       lattice uncertainties:

      BRBtaunumax=(1d0+4*dsqrt((dfB/fB)**2+dVub**2/Vub2
     .       +(dtauB/tauB)**2/4))*BRBtaunu
      BRBtaunumin=(1d0-4*dsqrt((dfB/fB)**2+dVub**2/Vub2
     .       +(dtauB/tauB)**2/4))*BRBtaunu


      prob(36)=0d0

      IF(BRBtaunumin.GE.BRBTAUNUexpmax)THEN
      PROB(36)=(BRBtaunumin-BRBTAUNUexpmax)/aux
      ENDIF
      IF(BRBtaunumax.LE.BRBTAUNUexpmin)THEN
      PROB(36)=(BRBtaunumax-BRBTAUNUexpmin)/aux
      ENDIF
...
*      b) epsilon'_b(t)
     ...
     .      SST**2*(H2(xTg2,xBg2)*CSB**2+H2(xTg2,xBg1)*SSB**2))
      ...
      aux=0d0
      do l=1,6
      aux=aux+1d0/MNEU(l)*N(l,4)*N(l,3)*
     .     (CST**2*(H2(xTneu(2,l),xBneu(1,l))*CSB**2+
     .      H2(xTneu(2,l),xBneu(2,l))*SSB**2)+
     .      SST**2*(H2(xTneu(1,l),xBneu(1,l))*CSB**2+
     .      H2(xTneu(1,l),xBneu(2,l))*SSB**2))
      enddo
...
*      C) epsilon_t(s)
     ...
     .     (CST**2*H2(xTg2,xQg)+SST**2*H2(xTg1,xQg))
      ...
      aux=0d0
      do l=1,6
      aux=aux+N(l,4)*N(l,3)/MNEU(l)*(
     .    CST**2*CSB**2*H2(ST(1)**2/MNEU(l)**2,SB(2)**2/MNEU(l)**2)
     .   +CST**2*SSB**2*H2(ST(1)**2/MNEU(l)**2,SB(1)**2/MNEU(l)**2)
     .   +SST**2*CSB**2*H2(ST(2)**2/MNEU(l)**2,SB(2)**2/MNEU(l)**2)
     .   +SST**2*SSB**2*H2(ST(2)**2/MNEU(l)**2,SB(1)**2/MNEU(l)**2))
      enddo
...
*      PMASS to the matching scale sc0)

      aux=0d0
      do i=1,3
      aux=aux+(SCOMP(i,2)-SCOMP(i,1)*tanb)
     .   *(SCOMP(i,1)+SCOMP(i,2)*epst3/tanb)/SMASS(i)**2
      enddo
      aux=aux+(PCOMP(3,2)-PCOMP(3,1)*tanb)
     .   *(PCOMP(3,1)+PCOMP(3,2)*epst3/tanb)/PMASS**2

      C70S0=1d0/18d0*MW**2/g2*MB0/(H1Q*(1d0+epst3))*sigRLbs*aux      

*       - Running to mt
      IF(PMASS.le.sc0)THEN
       IF(PMASS.ge.mb)THEN
       eta0=asc0/asf(PMASS)
       ELSE
       eta0=asmb/asc0
...
*  Dominant errors: HIG: Order (alphas**2) ~ 5%,
*  CHAR: Order(alphas) ~ 15%
                 
      DC70BSM=dsqrt((0.1d0*C7HIG)**2+
     .     (0.3d0*C7CHAR)**2+(0.3d0*C70S0)**2)
      DC80BSM=dsqrt((0.1d0*C8HIG)**2+
     .     (0.3d0*C8CHAR)**2+(0.3d0*C80S0)**2)
...
*      (Only for Bremsstrahlung and HQET Corrections)
     ...
     .    C80BSM*(ETA**(2d0/23d0)-ETA**(4d0/23d0))*8d0/3d0
      dKBSM0=dsqrt((dC70BSM*ETA**(4d0/23d0))**2+
     .    (dC80BSM*(ETA**(2d0/23d0)-ETA**(4d0/23d0))*8d0/3d0)**2)
...
*    BRSG ~ 3.15 10^(-4) [8] in the SM: to update
      z=(0.327d0)**2
     ...
     .    -VVtuim*(da(i)*afim(z)+db(i)*bfim(z))))
      dKCr=dKCr+eta**(aa(i))*(asmb/(4d0*pi)
     .    *VVtu*(da(i)*af(z)+db(i)*bf(z)))
      dKCi=dKCi+eta**(aa(i))*(asmb/(4d0*pi)
     .    *(-VVtuim*(da(i)*afim(z)+db(i)*bfim(z))))
      enddo
      dKC=dsqrt((dKCr*dVVtu)**2+(dKCi*dVVtuim)**2)
...
*   (errors coming from corrections ~ asmb/(4d0*pi) negligible):
      DKBSM=dsqrt(((1d0-2d0/9d0*asmb**2)*(eta**(4d0/23d0)))**2
     .      *DC70BSM**2+((1d0-2d0/9d0*asmb**2)*(8d0/3d0*(eta**(2d0/23d0)
     .      -eta**(4d0/23d0))))**2*DC80BSM**2)
...
*       5) Imaginary Parts of KC, KT, KBSM
     ...
     .   +VVtuim*(da(i)*af(z)+db(i)*bf(z)))
      dKCIMr=dKCIMr+eta**(aa(i))
     .   *(VVtu*(da(i)*afim(z)+db(i)*bfim(z)))
      dKCIMi=dKCIMi+eta**(aa(i))
     .   *(VVtuim*(da(i)*af(z)+db(i)*bf(z)))
      enddo
      dKCIM=asmb/(4d0*pi)*dsqrt((dKCIMr*dVVtu)**2+(dKCIMi*dVVtuim)**2)
...
**********************************************************************
***********************************************************************
*    Towards BR(b -> s gamma) including Electroweak corrections,
...
*   Now: delt=0.325:
      ff22p=-0.01445908619d0+.9889254405d0*dsqrt(z)-6.618735072d0*z
     .       +17.52172828d0*z*dsqrt(z)-6.886559723d0*z**2
     .       -60.56748284d0*z**2*dsqrt(z)+89.45278858d0*z**3
      ff22=ff22p*((1d0+VVtu)**2+VVtuim**2)+(VVtu**2+VVtuim**2)
     .   *(1d0/3d0*(1d0-(1d0-delt)**3)+1d0/2d0*(1d0-delt)**2)
      dff22=dsqrt(dVVtu**2*(ff22p*2d0*VVtu+2d0*ff22p+2d0*VVtu
     .   *(1d0/3d0*(1d0-(1d0-delt)**3)+1d0/2d0*(1d0-delt)**2))**2
     .   +(2d0*VVtuim*dVVtuim)**2*(ff22p
     .   +1d0/3d0*(1d0-(1d0-delt)**3)+1d0/2d0*(1d0-delt)**2))
      ff11=1d0/36d0*ff22
      ff12=-1d0/3d0*ff22
...
*      ff27=-0.190805d0+0.948865d0*dsqrt(z)-0.787805d0*z
*   Now: delt=0.325:
      ff27p=-0.04047347658d0-.4349867931d0*dsqrt(z)+2.997393353d0*z
...
*       b) Summing the Bremsstrahlung Contributions
     ...
     .      +ff88*C80b**2)
      dBREMS=asmb/pi*dsqrt((1d0/36d0*C10b**2*dff22)**2
     .      +(-1d0/3d0*dff22*C10b*C20b)**2
     .      +(-1d0/6d0*ff27p*C10b*C70b*dVVtu)**2
     .      +(1d0/18d0*ff27p*C10b*C80b*dVVtu)**2
     .      +(dff22*C20b**2)**2+(ff27p*C20b*C70b*dVVtu)**2
     .      +(-1d0/3d0*ff27p*C20b*C80b*dVVtu)**2)

***********************************************************************
*    D: Heavy Quark Effective Theory Corrections

      lambd2=0.12d0  ! = 1/4(M_B*^2-M_B^2)
      aux=MC*(1d0+asmc/pi*(-4d0/3d0))
      HQET=-lambd2/(9d0*aux**2)*(C20b-1d0/6d0*C10b)
     .                  *(KC0+rmu*(KT0+KBSM0))
      dHQET=rmu*dabs(-lambd2/(9d0*aux**2)*(C20b-1d0/6d0*C10b)*dKBSM0)

***********************************************************************
*  E: Finally: The Branching Ratio B -> Xs gamma

*     0) Parameters
      BRSL=0.1051d0 ! From [17], p. 127
      dBRSL=0.0013d0

      CCSL=0.546d0 ! [19] ([17]+[16]:0.5202; [17]+[14]:0.356)
      dCCSL=0.023d0
...
*      - Error estimate:
*     First: SM uncertainties  
*     + theoretical uncertainty (it includes NNLO +- 0.23 (from [8]):
     
     
      DBRSGmax=dsqrt((BRSG*dVtsVtb2/VtsVtb2)**2
     .     +(BRSG*dBRSL/BRSL)**2+(BRSG*dCCSL/CCSL)**2
     .     +(2d0*aux*(Ktot+EPSew)*dKC)**2+(2d0*aux*dKCIM)**2
     .     +(aux*dBREMS)**2)
     .     +2*0.1d0*BRSG
*     .     +2*aux*rmu*dsqrt(((Ktot+EPSew)*DKBSM)**2
*     .     +(KIM*2d0*asmb/9d0*eta**(2d0/23d0)*DC80BSM)**2
*     .     +((Ktot+EPSew)*(-ALEMMZ*dlog(MZ/MB)/pi)*dKBSM0)**2
*     .     +(dHQET/(2*rmu))**2)

      DBRSGmin=-dsqrt((BRSG*dVtsVtb2/VtsVtb2)**2
     .     +(BRSG*dBRSL/BRSL)**2+(BRSG*0.033d0/CCSL)**2
     .     +(2d0*aux*(Ktot+EPSew)*dKC)**2+(2d0*aux*dKCIM)**2
     .     +(aux*dBREMS)**2)
     .     -2*0.1d0*BRSG
*     .     -2*aux*rmu*dsqrt(((Ktot+EPSew)*DKBSM)**2
...
*      2.99 10^-4 < BR(B -> Xs gamma)=3.43 10^-4 < 3.87 10^-4
      ...
      IF(BRSGmin.GE.BRSGexpMax)THEN
      PROB(32)=BRSGmin/BRSGexpMax-1d0
      ENDIF
      IF(BRSGmax.LE.BRSGexpmin)THEN
...
*     1) Wilson Coefficients
     ...
     .      +29d0/189d0*(eta**(-28/23)-1d0))
      ...
      do i=1,3
       do j=1,3
       aux=aux+(SCOMP(i,2)-SCOMP(i,1)*tanb)*SCOMP(i,1)/cosb
     .       *(SCOMP(j,2)-SCOMP(j,1)*tanb)*SCOMP(j,1)/cosb
     .       *Intpropa(SMASS(i),WIDTH(i),SMASS(j),WIDTH(j),
     .             1d0/mb1s**2,6d0/mb1s**2)
       enddo
      enddo
      CQ12=(gg2/(2d0*MW)*(pi/(GF*MW))**2*mmu/MB0*sigRLbs)**2
      CQ12=(mBs/s2tw)**2*CQ12*aux
      aux=0d0
      aux=aux
     .      +(PCOMP(3,2)-PCOMP(3,1)*tanb)*PCOMP(3,1)/cosb
     .      *(PCOMP(3,2)-PCOMP(3,1)*tanb)*PCOMP(3,1)/cosb
     .      *Intpropa(PMASS,WIDTH(4),PMASS
     .    ,WIDTH(4),1d0/mb1s**2,6d0/mb1s**2)
      CQ22=(gg2/(2d0*MW)*(pi/(GF*MW))**2*mmu/MB0*sigRLbs)**2
      CQ22=(mBs/s2tw)**2*CQ22*aux

*     2) Branching Ratio between 1GeV^2 and 6GeV^2
      BRBSll1=1.59d-6+BRSL*4d0/CCSL*Vbsg*(ALEM0/(4d0*pi))**2
     .   *((8d0*dlog(6d0)-15d0/mb1s**2+215d0/(3d0*mb1s**6))
     .          *(C70**2-C70SM**2)
     .    +3d0/2d0*(cQ12+cQ22))

      BRBSll1max=(BRBSll1-1.59d-6)*1.6d0+(1.59d0+.22d0)*1.d-6
      BRBSll1min=(BRBSll1-1.59d-6)*.4d0+(1.59d0-.22d0)*1.d-6

*     3) Comparison with experiment
*     2 sigma bounds from Huber, Hurth, Lunghi, 0712.3009:
*   0.60 10^-6 < BR(B -> Xs l+l-)=1.60 10^-6 < 2.60 10^-6
      prob(40)=0d0
      IF(BRBSll1min.GE.2.6d-6)THEN
      PROB(40)=DABS(BRBSll1min-2.6d-6)*1.D6
      ENDIF
      IF(BRBSll1max.LE.0.6d-6)THEN
      PROB(40)=DABS(BRBSll1max-0.6d-6)*1.D6
      ENDIF
...
*     4) High M_{l+l-} region: 14.4 GeV^2 < M_{l+l-}^2 < mb^2
     ...
     .      +(PCOMP(3,2)-PCOMP(3,1)*tanb)*PCOMP(3,1)/cosb
     .      *(PCOMP(3,2)-PCOMP(3,1)*tanb)*PCOMP(3,1)/cosb
     .   *Intpropa(PMASS,WIDTH(4),PMASS,WIDTH(4),14.4d0/mb1s**2,1d0)
      CQ22=(gg2/(2d0*MW)*(pi/(GF*MW))**2*mmu/MB0*sigRLbs)**2
      CQ22=(mBs/s2tw)**2*CQ22*aux

      BRBSll2=2.40d-7+BRSL*4d0/CCSL*Vbsg*(ALEM0/(4d0*pi))**2
     .   *((8d0*dlog(mb1s**2/14.4d0)-4d0*(1d0-14.4d0/mb1s**2)
     .    +4d0/3d0*(1d0-(14.4d0/mb1s**2)**3))*(C70**2-C70SM**2)
     .    +3d0/2d0*(cQ12+cQ22))

      BRBSll2max=(BRBSll2-2.40d-7)*1.6d0+2.40d-7*(1d0+2d0*.29d0)
      BRBSll2min=(BRBSll2-2.40d-7)*.4d0+2.40d-7*(1d0-2d0*.26d0)

*     2 sigma bounds from Huber, Hurth, Lunghi, 0712.3009:
*     2.0 10^-7 < BR(B -> Xs l+l-)=4.4 10^-7 < 6.8 10^-7

      IF(BRBSll2min.GE.6.8d-7)THEN
      PROB(40)=PROB(40)+DABS(BRBSll2min-6.8d-7)*1.D7
      ENDIF
      IF(BRBSll2max.LE.2.0d-7)THEN
      PROB(40)=PROB(40)+DABS(BRBSll2max-2.0d-7)*1.D7
      ENDIF


* For comparison: the Pole contribution only (from Hiller):
      BRBSLL=BRSL/CCSL*Vbsg*3d0*pi/(2d0*dsqrt(2d0)*GF)
     .      *mmu**2*PMASS/(mb1s**8*WIDTH(4))
     .      *(mb1s**2-PMASS**2)**2
     .   *sigRLbs**2*(PCOMP(3,2)-PCOMP(3,1)*tanb)**2
     .   *(PCOMP(3,1)/cosb)**2*(mBs/MB0)**2
