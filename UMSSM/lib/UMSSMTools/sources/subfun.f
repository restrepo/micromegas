...
c+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	double precision function A0(m2,q)

	implicit none
	double precision m2,q
	if(m2.ne.0.d0) then
	 A0 = m2 * (1.d0-dlog( m2/q )) 
	else
	 A0 = 0.d0
	endif
	end

	double precision function B0(p,m1,m2,q)
...
* zero momentum
	  ...
	  be0 = -log(m2/mudim2)
        ...
        B0 = dble(be0 + divergence)

	end


	double precision function B1(s,mi,mj,q)

* note: all input is quadratical: s=p^2, mi=m1^2, mj=m2^2, q=q^2

	implicit none
	double precision s,mi,mj,B0,A0,q

	if(mi.eq.mj) then
	 B1 = B0(s,mi,mj,q)/2.D0
	else
	 B1= (A0(mj,q) - A0(mi,q)
     .	   + (s+mi-mj)*B0(s,mi,mj,q))/(2.D0*s) 
	endif
	end
