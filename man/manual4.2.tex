...
\sum_{\substack{
\alpha ,\beta }}  p_{\alpha\beta}^2(s) g_{\alpha} g_{\beta} \\
&&\big(
\sum_{\substack{ x ,y }}  
 {\sigma}_{\alpha\beta\to x y}(s)+ \frac{1}{2}\sum_{\substack{x  \gamma }}
 {\sigma}_{\alpha\beta\to x\gamma}(s)
  \big)
\label{sigmaV}\\
\overline{n}(T)&=&\frac{T}{2\pi^2 } \sum_{\alpha} g_\alpha m^2_\alpha K_2(\frac{m_\alpha}{T}),
\end{eqnarray}
Here  $\alpha$, $\beta$, $\gamma$  is used for  {\tt Odd} particles and $x$,$y$
for {\tt Even}  particles.    ${\sigma}_{\alpha\beta\to x[\gamma/y]} $ is the cross section for the 
...
$m_\alpha$ and $g_\alpha$ stand for the  mass and the number of degrees of freedom of particle $\alpha$.
...
\noindent
 $\bullet$    \verb|vSigmaCC(T,cc)|\\
  calculates  the thermally  averaged  $cross\; section
\times velocity$ for $2\to2$ and $2\to3$ processes. \verb|T| is the temperature in
[GeV],  $cc$ is the address of the code for each process. This adress
can be obtained by the function {\tt newProcess} presented in  Section
\ref{cross_section}. The returned value  is given in {\tt [pb]}. {\tt vSigmaCC} is defined by
the integral
$$ <v \sigma^{\alpha\beta\to X}>_T=  \frac{1}{T m_\alpha^2 m_\beta^2
K_2(\frac{m_a\alpha}{T})K_2(\frac{m_\beta}{T})} \int d\sqrt{s}
K_1(\frac{\sqrt{s}}{T})p_{cm}^2(s)\sigma^{\alpha\beta\to X}(p_{cm}(s))$$  
...
vSigma} described  above when there is an important NLSP contribution to the total
...
Thus a small difference in initial abundances can lead to a  large DM asymmetry after freeze-out as is the case for the  baryonic
asymmetry.\\
...
micrOMEGAs Makefile will link it automatically. The list of available LHAPDF
...
setLHAPDF terminates the executable.
...
Some routines for computing constraints were taken from the {\tt MSSM} and were adapted to the UMSSM :
...
as other routines in the UMSSM include this constraint (see below).\\

Two other routines in C are included :

\noi$\bullet$ \verb|Zinv()|\\
returns 1 and prints a WARNING if the invisible width of the $Z$ boson of the Standard Model ($Z_1$ in the UMSSM)
is larger than 0.5 MeV (limit from~\cite{Freitas:2014hra}) and it returns 0 if this constraint is fulfilled.
...
%where the $Z$ boson is at least uniquely defined by its PDG=23 and where the neutral LSP is its own antiparticle or not.

\noi$\bullet$ \verb|Zprimelimits()|\\
returns 1 if the mass of the $Z_2$ boson is excluded by a combination of CMS~\cite{Khachatryan:2014fba} and ATLAS~\cite{Aad:2014cka}
limits on the $Z'$ mass obtained with 20 fb$^{-1}$ at $\sqrt{s}$ = 8 TeV and a rescaling to take into acccount
UMSSM effects. It returns 0 if this constraint is fulfilled.\\
...
\bibitem{Honda:2006qj}
  ...
  %``Calculation of atmospheric neutrino flux using the interaction model
  %calibrated with atmospheric muon data,''
  Phys.\ Rev.\ D {\bf 75} (2007) 043006
  [astro-ph/0611418].
  %%CITATION = ASTRO-PH/0611418;%%

%\cite{Blennow:2007tw}

\bibitem{Blennow:2007tw}
  M.~Blennow, J.~Edsjo and T.~Ohlsson,
  %``Neutrinos from WIMP annihilations using a full three-flavor Monte
  %Carlo,''
  JCAP {\bf 0801} (2008) 021
  [arXiv:0709.3898 [hep-ph]].
  %%CITATION = ARXIV:0709.3898;%%
  %104 citations counted in INSPIRE as of 11 juil. 2015

%\cite{Baratella:2013fya}
\bibitem{Baratella:2013fya}
  P.~Baratella, M.~Cirelli, A.~Hektor, J.~Pata, M.~Piibeleht and A.~Strumia,
  %``PPPC 4 DM$\nu$: a Poor Particle Physicist Cookbook for Neutrinos from
  %Dark Matter annihilations in the Sun,''
  JCAP {\bf 1403} (2014) 053
  [arXiv:1312.6408 [hep-ph]].
  %%CITATION = ARXIV:1312.6408;%%
  %11 citations counted in INSPIRE as of 11 Jul 2015


\bibitem{Chirkin:2004hz}
  D.~Chirkin and W.~Rhode,
  %``Muon Monte Carlo: A High-precision tool for muon propagation through
  %matter,''
  hep-ph/0407075.
  %%CITATION = HEP-PH/0407075;%%
  %103 citations counted in INSPIRE as of 11 Jul 2015

%\cite{Freitas:2014hra}
\bibitem{Freitas:2014hra}
  A.~Freitas,
  %``Higher-order electroweak corrections to the partial widths and branching ratios of the Z boson,''
  JHEP {\bf 1404} (2014) 070
  [arXiv:1401.2447 [hep-ph]].
  %%CITATION = ARXIV:1401.2447;%%
  %21 citations counted in INSPIRE as of 30 Sep 2015

%\cite{Khachatryan:2014fba}
\bibitem{Khachatryan:2014fba}
  ...
  JHEP {\bf 1504} (2015) 025
  [arXiv:1412.6302 [hep-ex]].
  %%CITATION = ARXIV:1412.6302;%%
  %53 citations counted in INSPIRE as of 30 Sep 2015
  
%\cite{Aad:2014cka}
\bibitem{Aad:2014cka}
  G.~Aad {\it et al.} [ATLAS Collaboration],
  %``Search for high-mass dilepton resonances in pp collisions at $\sqrt{s}=8$??TeV with the ATLAS detector,''
  Phys.\ Rev.\ D {\bf 90} (2014) 5,  052005
  [arXiv:1405.4123 [hep-ex]].
  %%CITATION = ARXIV:1405.4123;%%
  %126 citations counted in INSPIRE as of 30 Sep 2015  

%\cite{DaSilva:2013jga}
\bibitem{DaSilva:2013jga}
  J.~Da Silva,
  %``Supersymmetric Dark Matter candidates in light of constraints from collider and astroparticle observables,''
  arXiv:1312.0257 [hep-ph].
  %%CITATION = ARXIV:1312.0257;%%
  %2 citations counted in INSPIRE as of 30 Sep 2015
 
%\cite{Belanger:2015cra}
\bibitem{Belanger:2015cra}
  G.~BŽlanger, J.~Da Silva, U.~Laa and A.~Pukhov,
  ...
  %%CITATION = ARXIV:1505.06243;%%
  %2 citations counted in INSPIRE as of 30 Sep 2015
  
  \bibitem{Bernon:2015hsa}
  J.~Bernon and B.~Dumont,
  %``Lilith: a tool for constraining new physics from Higgs measurements,''
  Eur.\ Phys.\ J.\ C {\bf 75} (2015) 9,  440
  [arXiv:1502.04138 [hep-ph]].

  
  \bibitem{Belanger:2015hra}
  G.~Belanger, J.~Da Silva, T.~Perrillat-Bottonet and A.~Pukhov,
  %``Limits on dark matter proton scattering from neutrino telescopes using micrOMEGAs,''
  arXiv:1507.07987 [hep-ph].
  %%CITATION = ARXIV:1507.07987;%%
