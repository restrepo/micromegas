...
\verb| LLL_singlet/       | Simplified model with singlet charged lepton and real scalar DM \\ 
...
The packages included in micrOMEGAs distribution are:
 ...
 {\tt Lilith} that checks the Higgs properties,  and {\tt LanHEP} that generates model
 file.  
...
\verb|Mcdm2|.  For models with only one DM candidate, \micro\ will set  \verb|CDM2|=NULL and \verb|Mcdm2|=0 (in Fortran  the string  is filled by space symbols). 
...
the form factor for the same isotope, the  form factors SxxXe129M and SxxXe131Me were
added recently,  see details in~\cite{Belanger:2008sj}.
...
\verb|dNdE| which  has \verb|NRDIM| elements with \verb|NRDIM = 150|.   The value in the $n^{th}$ element corresponds to
$$
dNdE[n] = \frac{dN}{dE}|_{E= 0.01 \cdot 1.08^{n}[eV]}
$$
in units of ${\rm (1/keV/kg/day)}$.\\ 
...
An auxiliary routine is provided to work with the energy
...
depend implicitly on the  global parameter {\tt Mcdm} except for {\tt basicSpectra} and {\tt basicNuSpectra} . These routines do not   take
...
The {\tt MODEL} directory contains both  C and FORTRAN samples of {\it main}
...
\noindent$\bullet$ \verb|displayPlot(title,xName,xMin,xMax,lScale,  N, ...)|\footnote{This function is not available in Fortran}\\
