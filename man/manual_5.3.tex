...
{ \color{blue}
   Graphical interface of CalcHEP and micrOMEGAs uses X11 routines. To compile corresponding code one
need X11 header files \\
\begin{verbatim}
       /use/include/X11/*.h
\end{verbatim}
If these files are absent on your computer, you have to install the {\it X11-devel} package
The name of package depends on operation system:
\begin{verbatim}
...
\noindent$\bullet$ \verb|gEff(T)|\\
 returns the effective number of degrees of freedom for the energy density of radiation at a bath temperature \verb|T|,  only SM particles are
included:
{\color{blue} 
\begin{equation}
  \rho_R(T) = \frac{\pi^2}{30} g_{eff}(T) T^4
\end{equation}
}
 \\
\noindent$\bullet$ \verb|hEff(T)|\\ returns the effective number of degrees of freedom for the entropy density of radiation at a bath temperature
\verb|T|:
{\color{blue}
 \begin{equation}
    s(T)=\frac{2\pi^2}{45} h_{eff} T^2
\end{equation}
}
\\
\noindent$\bullet$ \verb|hEffLnDiff(T)|\\ returns the derivative of $h_{eff}$ with respect to the
{\color{blue} {\it log}} of bath temperature, $\frac{d\log(h_{eff}(T))}{d\log(T)}$.\\
...
\\}
{\color{blue} 
{\tt Hubble} is defined via density of Universe taking into account  radiation,
dark energy and contribution of dark matter with baryonic matter.
\begin{eqnarray}
   H&=& \sqrt{\frac{8\pi \rho(T)}{ 3 M_P^2}}    \\
   \rho(T)&=&  \frac{\pi^2}{30} g_{eff}(T) T^4 + \mu_{DM}\frac{2\pi^2}{45} h_{eff}(T) T^3 + \mu_{DE}^4 
\end{eqnarray}
where $M_P$   is the Planck mass,  $\mu_{DM}=0.521$ eV,   $\mu_{DE}=2.24\;10^{-3}$ eV.
 The
...
calcualtes time interval in   seconds  during which temperature of Universe
decreases from {\tt T1[GeV]}  to {\tt T2[GeV]}.    It is obtained by integration of 
Hubble rate. 
\begin{equation}
  t= \int \limits_{T_2}^{T_1}  \left(1+ \frac{1}{3}
\frac{d\log(h_{eff}(T))}{d\log(T)}\right)\frac{dT}{H(T) T} 
\end{equation}
The current temperature 2.73K is presented by constant
\verb|T2_73K|, so,  the age of Universe can be calculated as
\verb|Hubble(10,T2_73K)|. By this way we get the age of Universe 13.794  Gyr, which is close to
PDG  estimation 13.797(23) Gyr.  In the same time {\tt Tmreq} corresponds to
time $t_{eq}$=50.1 kyr, which is close to PDG number 51.1(8) kyr. \\
}
%
{\color{blue}
\noindent$\bullet$ \verb|freeStreaming(p/m, T1,T2)|\\
calculates length of   trajectory in {\tt Mpc} units   of free propagated particle started at
{\tt T1[GeV]} and finished at {\tt T2}. The {\tt p/m} parameter
characterizes initial velocity of particle $v=\frac{p/m}{\sqrt{ 1+ (p/m)^2}}$.
Usually free streaming  is presented by integral over time 
\begin{equation}
   \int \limits_{t_2}^{t_1}  \frac{v(t) dt}{ a(t)}
\end{equation}  
where $a(t)$ is the scale factor. For calculation we present  this integral
as integral over T. In particular,  $ a = \left( s(T2\_73K)/s(T)\right)^{\frac{1}{3}}$   
}\\  
%
...
{\color{blue}
\subsection{ {\tt Tstart, Tend} and  error codes for routines calculating relic density} 
{ \tt darkOmega[TR], darkOmega2[TR], darkOmegaN[TR]}  calculate  DM relic
density solving differential equation for abundance 
$$  Y= n/s $$ 
where $n$ is number density of DM and $s$ is entropy density. The equation
contains thermal equilibrium  abundance $\bar{Y}(T)$ and  $DM\to SM$
annihilation cross section $\sigma_v(T)$. Abundance is calculated   at temperature {\tt Tend} defined 
by the user. Default value is 
Tend=$10^{-3}${\tt GeV}. Usually evolution of relic density stops at higher
temperature. The {\tt TR } routines have input parameters which specify initial
temperature and  abundances. This initial temperature is assigned to
variable {\tt Tstart}. Other routines define parameter {\tt Tstart} by
condition 
$$ Y(Tstart)-\bar{Y}(Tstart) \approx 0.1 \bar{Y}(Tstart) $$ 

 The {\tt darkOmega*} routines have $\&err$ parameter which returns the error code of routine.
In particular, if {\tt Tstart} is not detected,  $err=64$. Mainly it means that DM nether was in thermal equilibrium
with SM particles.  In case of problem with  solution of differential
equation the error code is  $128$. It can appear if {\tt Tstart} is too large and
there is a {\it stiff} problem of solution of differential equation.


For calculation $\sigma_v(T)$ we use program {\it simpson} to evaluate
integral over scattering angle and energy of collision. This
program can return an  error code 
\begin{verbatim} 
1 - NAN in integrand;
2 - too deep recursion;
4 - loss of precision.
\end{verbatim}
which is  passed to $err$.
...
This trick is  explained  in section~\ref{Simpson}. The error code {\tt err} is a binary code which can signal several problems simultaneously. 
...
temperature interval {\tt [Tff,Tstart]}  defined by the
conditions $Y(T_{start})\approx 1.1Y_{eq}(T_{start})$,
$Y(T_{ff})\approx10Y_{eq}(T_{ff})$. For temperatures below $ T_{ff}$, the contribution of  $Y_{eq}$
 is neglected and  the differential equation is integrated
explicitly in interval [Tff, Tend]{\color{blue} , where parameter {\tt Tend} is defined by the
user}. The solution in the interval {\tt [Tff,Tstart]}  interval is tabulated and can be displayed  via the
...
need to be divided into \emph{sectors}, within each of which chemical equilibrium is %expected. 
observed.  
...
%  \textcolor{blue}{ {\tt checkTE}  applies the   linear part  of evolution equation  for  abundance  for   subset of  the $n^{th}$  thermal set and
  %     checks  how fast it will be in chemical equilibrium with its addition.  At this approximation one expects  $dY(t)/dt= \omega (Y_{eq}-Y)$. }
...
%Note that to calculate $\langle v\sigma \rangle$ for processes with incoming bath particles, \micro uses a short cut to avoid calculating $\bar{n}_0$ for bath particles,  instead we substitute $\bar{n}_0=s$. Thus  $\langle v\sigma_{2010} \rangle$ in Eq.~\ref{eq:sigmav} is rescaled, to compensate for this factor  the rate of co-scattering processes  is defined as 
%\begin{equation}     
  %  \Gamma_{2\to 1}= \verb|vSigmaN(T,"2010")| s(T) \,.
%\end{equation}

%Note that to convert  $\Gamma_{2\to1}$  with the decay width of particle "2", one has to do the proper  
%transformation of units,  
%\begin{equation}
 % \Gamma_{2\to1}[{\rm GeV}] =   vSigmaN(T,"2010") s(T) \frac{1}{3.894\times 10^8} \,
 % \end{equation}\\
 % \textcolor{green}{Sasha please say what is the final solution adopted in the code}

To find the contribution of  different processes to  {\tt vSigmaN},  one can call
\\
%$\bullet$~\verb|vSigmaNCh(T, channel, n, process)|  which returns the contribution of $n^{th}>0$ process and
%writes out the process name in the text parameter  {\it process}. When {\it n} exceeds the number of processes,
%the string  {\it process} remains empty.

%\textcolor{blue}{for comparison channel information generated by vSigmaCh and vSigmaNch 
\noindent
