...
*/  
err_code=0;
  r=(cc23->interface->sqme)(1,pvect,&err_code)*pcm1*pcm2/(128*M*pmass[0]*pmass[0]*M_PI*M_PI*M_PI);
if(err_code) printf("cos=%E  errcode=%d\n", csfi,err_code);  
...
static double Spectra22A(char*name1,char*name2,double ** Spectra,txtList plusA)
{
  ...
  { 
     ...
     for(i=1;i<=cc23->interface->nvar;i++) if(cc23->link[i]) cc23->interface->va[i]=*(cc23->link[i]); 
     if( cc23->interface->calcFunc()>0 )  continue;
...
static void getSpectrum(int wPol, double M, double m1,double m2,char*n1,char*n2, long N1, long N2,int outP, double *tab)
{
  int i;
  int inP=-1;
  long N;
...
/*        if(PrintOn && outP==0 ) printf(" %.0f%% transverse W produced\n",100*fabs(left+right));*/
  ...
  { char* nn[2];
    double mm[2];
    double E[2]; 
    double p2=sqrt((M*M-(m1+m2)*(m1+m2))*(M*M-(m1-m2)*(m1-m2)))/(2*M);
    int k;
    ...
    mm[0]=m1;
    mm[1]=m2;
    E[0]=sqrt(m1*m1+p2*p2);
    E[1]=sqrt(m2*m2+p2*p2);
